...
class MxViewController
    <I: MxIntent, R: MxResult, VS: MxViewState, VM: MxViewModel<I, R, VS>>: UIViewController, BundleSender, Mvi {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let bundleSender = sender as? BundleSender {
            if let bundle = bundleSender.getDestinationBundle() {
                if segue.identifier == bundle.identifier {
                    if let destinationBundleSender = segue.destination as? BundleSender {
                        destinationBundleSender.setDestinationBundle(bundle: bundle)
                    }
                }
            }
        }
